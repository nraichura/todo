...
class ItemApisTest {
  ...
  void updateDescription_NotFoundResponse() throws UnsupportedEncodingException {
    ...
    List<String> errors =
        Objects.requireNonNull(response.getBody()).errors().stream()
  ...
  void updateStatus_ShouldRunSuccessfully() throws UnsupportedEncodingException {
    ...
    ResponseEntity<ItemDto> response =
        updateStatus(ItemStatus.DONE, String.valueOf(itemId), ItemDto.class);
  ...
  @Test
  void updateStatus_ShouldNullifyMarkedDoneAt_WhenStatusUpdateFromDoneToNotDone()
    ...
    // Act
    ResponseEntity<ItemDto> response2 =
        updateStatus(ItemStatus.NOT_DONE, String.valueOf(itemId), ItemDto.class);
  ...
  @Test
  void updateStatus_ShouldRespondWithBadRequest_WhenStatusUpdateToPastDue()
      throws UnsupportedEncodingException {
    // Prepare - create an item
    ItemDto newItem = createItem("desc", Instant.now().plus(1, ChronoUnit.HOURS));
    List<String> expectedResponse =
        List.of(
            String.format(
                "Changing item status for the item id '%s' to 'Past Due' is not allowed.",
                newItem.getId()));
    // Extract the created item's ID
    long itemId = newItem.getId();
    ResponseEntity<GenericErrorModel> response =
        updateStatus(ItemStatus.PAST_DUE, String.valueOf(itemId), GenericErrorModel.class);
  ...
  @Test
  void getItems_ShouldReturnSpecificItems_WhenFilterCriteriaIsProvided()
      throws UnsupportedEncodingException {
    // Prepare - create an item
    ItemDto newItem = createItem("desc", Instant.now().plus(1, ChronoUnit.HOURS));
    ItemDto newItem2 = createItem("desc2", Instant.now().plus(1, ChronoUnit.HOURS));
    ...
    // Act
    String url =
        String.format(BASE_URL + GET_ITEMS_ENDPOINT_URL + "?status=%s", ItemStatus.NOT_DONE);
    ...
    await()
        ...
        .untilAsserted(
            () -> {
              ResponseEntity<ItemDto> response = getItem(itemId);
              assertEquals(
                  ItemStatus.PAST_DUE, Objects.requireNonNull(response.getBody()).getStatus());
            });
  }

  private ResponseEntity<ItemDto> getItem(Long itemId) throws UnsupportedEncodingException {
    return template.exchange(
        BASE_URL + GET_ITEM_ENDPOINT_URL,
        HttpMethod.GET,
        null,
  ...
  private ItemDto createItem(String desc, Instant dueAt) {
    ResponseEntity<ItemDto> createResponse =
        template.postForEntity(
            BASE_URL + CREATE_ITEM_ENDPOINT_URL,
            new CreateItemRequestDto(desc, dueAt),
            ItemDto.class);
  ...
  private <T> ResponseEntity<T> updateStatus(
      ItemStatus status, String itemId, Class<T> responseType) throws UnsupportedEncodingException {
    return template.exchange(
        BASE_URL + STATUS_UPDATE_ENDPOINT_URL,
        HttpMethod.PATCH,
        new HttpEntity<>(new ChangeStatusRequestDto(status)),
        responseType,
        urlEncode(itemId));
  }

  private <T> ResponseEntity<T> updateDescription(long itemId, Class<T> responseType)
      throws UnsupportedEncodingException {
    return template.exchange(
        BASE_URL + DESCRIPTION_UPDATE_ENDPOINT_URL,
        HttpMethod.PATCH,
        new HttpEntity<>(new ChangeDescriptionRequestDto("newDesc")),
        responseType,
        urlEncode(String.valueOf(itemId)));
  }
